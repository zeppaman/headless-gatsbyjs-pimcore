    ...
    {edges.map((edge) => (
      <div className={styles['feed__item']} key={edge.node.slug}>
        <div className={styles['feed__item-meta']}>
          <time className={styles['feed__item-meta-time']} dateTime={ new Date(edge.node.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}>
          { new Date(edge.node.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}
          </time>
